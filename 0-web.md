...
The content of the world wide web is made up of code:
...
When you access a website, the owner's server sends your computer the code which your browser then renders into a web page that you can view and interact with.
...
On another web page, right click on any element in the page and select "Inspect" or "Inspect Element" to open your browser's built in **developer tools**. 
...
These are essentially identifiers used to ask servers for specific pages or content. 
...
protocol `://` subdomain `.` domain `.` top-level domain `/` path and filename `?` query with parameters `#` fragment or anchor

Each component of the URL plays a part in the content that is eventually displayed in your browser, so it is helpful to understand the pieces:
...
- **Path and filename:** these can be imagined like folders and files as located on the server designated by the domain. For example, `/about/dogs.html`, is the file "dogs.html" in the "about" folder. If no filename is given, by default the server provides the file named `index`.
...
Honestly, to start understanding and developing websites you don't need to know much more about how the web works.
Familiarize your self with the parts of a URL, learn a bit about HTML and CSS, and you are already understanding the fundamentals of the web!

For more details about how the web works:
